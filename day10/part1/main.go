...
func main() {
	...
	for _, trail := range trailhead_map {
		...
		search(mountain_map, rows, cols, trail[0], trail[1], visited)
		point_res := len(visited)
...
func search(mountain_map [][]int, rows int, cols int, curr_r int, curr_c int, visited map[int]struct{}) {
	if mountain_map[curr_r][curr_c] == 9 {
		visited[curr_r*cols+curr_c] = struct{}{}
		return
	}
	for _, dir := range dirs {
		...
		if mountain_map[new_r][new_c] == mountain_map[curr_r][curr_c]+1 {
			search(mountain_map, rows, cols, new_r, new_c, visited)
		}
	}
}
